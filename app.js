...
let tiebreakQueue = []; // 残りの対決キュー [['D','I'], ['勝者','S']] のように積む
let tiebreakScores = {D:0,I:0,S:0,C:0}; // 追加質問の得点（元スコアとは別管理）
...
// 3タイプ同点の場合の対決順序
const TIEBREAK_ORDER_3 = {
  'DIS': [['D','I'], null], // null = 勝者 vs S（動的に決める）
  'DIC': [['D','I'], null], // 勝者 vs C
  'DSC': [['D','S'], null], // 勝者 vs C
  'ISC': [['I','S'], null]  // 勝者 vs C
};
...
function start(){
  idx=0; isTiebreak=false; tiebreakQueue=[]; tiebreakScores={D:0,I:0,S:0,C:0};
...
function renderTiebreakQuestion(){
  if(tiebreakQueue.length===0){ showResult(); return; }
  const [tA, tB] = tiebreakQueue[0];
  const key=[tA,tB].sort().join('');
  const q=TIEBREAK_Q[key];
  if(!q){ tiebreakQueue.shift(); renderTiebreakQuestion(); return; }

  const totalQ = DATA.questions.length;
  const tbDone = DATA.questions.length - tiebreakQueue.length; // 何問目の追加質問か（近似）
  $qIndex.textContent = totalQ + (Object.keys(TIEBREAK_Q).length - tiebreakQueue.length + 1);
  $bar.style.width='97%';
  $back.disabled=true;
  $next.disabled=true;

  $qText.innerHTML=`<span style="font-size:0.78em;color:var(--accent,#f071a8);display:block;margin-bottom:6px;">🌸 同点のため追加質問（${tA}・${tB}タイプが同点）</span>${q.text}`;
  $choices.innerHTML='';
  ...
  q.choices.forEach(c=>{
    ...
    div.addEventListener('click',()=>{
      ...
      div.style.outline=`2px solid ${COLORS[c.type]}`;
      // 勝者のスコアを加算
      tiebreakScores[c.type]++;
      const winner=c.type;
      const loser=(tA===winner)?tB:tA;
      tiebreakQueue.shift();
      // キューに次の対決があれば勝者を差し込む
      if(tiebreakQueue.length>0){
        const next=tiebreakQueue[0];
        // nullプレースホルダーを勝者に置き換え
        if(next[0]===null) tiebreakQueue[0]=[winner, next[1]];
        else if(next[1]===null) tiebreakQueue[0]=[next[0], winner];
      }
      setTimeout(()=>{
        if(tiebreakQueue.length===0){
          // 追加質問の結果を元スコアに反映して結果表示
          Object.keys(tiebreakScores).forEach(t=>{ scores[t]+=tiebreakScores[t]; });
          showResult();
        } else {
...
function buildTiebreakQueue(tied){
  // 2タイプ同点
  if(tied.length===2){
  ...
  if(tied.length===3){
    ...
    const order=TIEBREAK_ORDER_3[key];
    if(order){
      // 第2戦は勝者 vs 残り1タイプ（nullで後で埋める）
      const remaining=tied.filter(t=>!order[0].includes(t))[0];
      return [[order[0][0],order[0][1]], [null, remaining]];
    }
    // フォールバック：先頭2タイプ vs 残り
    const remaining=tied.slice(2)[0];
    return [[tied[0],tied[1]], [null, remaining]];
  }
  return [];
}
...
function next(){
  ...
  if(idx<DATA.questions.length-1){ idx++; renderQuestion(); }
    ...
    if(tied.length>=2){
      isTiebreak=true;
      tiebreakQueue=buildTiebreakQueue(tied);
      tiebreakScores={D:0,I:0,S:0,C:0};
      $back.disabled=true;
      $next.disabled=true;
      renderTiebreakQuestion();
    } else {
...
function drawChart(sc, animate=true){
  ...
  const keys=['D','I','S','C']; const max=DATA.questions.length;
  ...
  const step=()=>{
    progress=Math.min(1,progress+0.08); ctx.clearRect(0,0,$chart.width,$chart.height);
    keys.forEach((k,i)=>{
      const x=xStart+i*(w+gap); const h=targetHeights[i]*progress;
      ctx.fillStyle='rgba(240,113,168,0.12)'; ctx.fillRect(x,baseY-180,w,180);
      ctx.fillStyle=COLORS[k]; const y=baseY-h,r=10;
      ctx.beginPath(); ctx.moveTo(x,y+r); ctx.arcTo(x,y,x+r,y,r); ctx.lineTo(x+w-r,y);
...
function renderMissions(){
  ...
  items.forEach((m,i)=>{
    const id='m'+i; const wrap=document.createElement('label'); wrap.className='choice';
    wrap.innerHTML=`<input type="checkbox" id="${id}"> ${m}`;
